                ...
                <div>
                    <div className="flex flex-col mt-5">
                        {adminSidebar.map((item, index) => {
                            ...
                            return (
                                <Link key={index} onClick={closeSidebar} to={Array.isArray(item.url) ? item.url[0] : item.url} className={`flex items-center gap-2 p-2 transition-all rounded-lg ${isActive ? 'bg-blue' : ''}`}><item.Icon className="text-lg" /><span>{item.title}</span></Link>
