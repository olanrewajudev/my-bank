...
const OTHER_COUNTRIES = [
  'Nigeria', 'Ghana', 'Kenya', 'South Africa', 'Canada', 'Australia', 'India', 'China',
  'Japan', 'Brazil', 'Mexico', 'United Arab Emirates', 'Saudi Arabia', 'Singapore',
  'Philippines', 'Indonesia', 'Egypt', 'Turkey', 'Argentina', 'New Zealand', 'Other',
]
...
const initialWithdrawForm = {
  ...
  accName: '',
  bank: '',
  recieveracctnumber: '', // used as account number for USA / Other
  routineNumber: '',      // USA only
  iban: '',                // Europe only
  swiftCode: '',           // Europe / Other
  amount: '',
}
type WithdrawField = keyof typeof initialWithdrawForm
...
export default function Transfer() {
  ...
  const handleCountryChange = (value: string | null) => {
    ...
    setForm((prev) => ({
      ...prev,
      country: value || '',
      // clear region-specific fields when switching regions
      ...
      <Modal size="32rem" centered withCloseButton={false} opened={opened} onClose={close}>
        <div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSubmitWithdraw() }}>
            ...
            {region === 'europe' && (
              <>
                <Forminput error={formError.iban} content="IBAN" placeholder="e.g. DE89 3704 0044 0532 0130 00" value={form.iban} onChange={handleChange('iban')}/>
                <Forminput error={formError.swiftCode} content="SWIFT / BIC Code" placeholder="e.g. DEUTDEFF" value={form.swiftCode} onChange={handleChange('swiftCode')}/>
              </>
            )}
            ...
            {region === 'other' && (
              <>
                <Forminput error={formError.recieveracctnumber} content="Account Number" type='number' placeholder="Account Number" value={form.recieveracctnumber} onChange={handleChange('recieveracctnumber')}/>
                <Forminput error={formError.swiftCode} content="SWIFT / BIC Code" placeholder="e.g. FTBCUS3P" value={form.swiftCode} onChange={handleChange('swiftCode')}/>
              </>
            )}
