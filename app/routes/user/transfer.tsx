...
function groupByDate(transactions: Transaction[]) {
  ...
  transactions.forEach((tx) => {
    const key = tx.date
    if (!groups[key]) groups[key] = []; groups[key].push(tx)
  })
  // newest date first
      ...
      <Modal size="32rem" centered withCloseButton={false} opened={opened} onClose={close}>
        <div>
          ...
          <form onSubmit={(e) => { e.preventDefault() }}>
            <Forminput error='' content="Routing Number" placeholder="Routing Number" />
            <Forminput error='' content="Bank Name" placeholder="Bank Name" />
            <Forminput error='' content="Account Number" placeholder="Account Number" />
            <Forminput error='' content="Account Holder Name" placeholder="Account Holder Name" />
            <Forminput error='' content="Amount" placeholder="$0.00" type="number" />
            <Forminput error='' content="Description (Optional)" placeholder="Description" />
            <Formbutton title="Submit Withdrawal" />
          </form >
        </div>
      </Modal>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <span
                        className={`mt-1 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full ${isWithdrawal
                            ? 'bg-red-100'
                            : 'bg-emerald-100'
                          }`}
                      >
                        <HiOutlineArrowPath
                          className={`text-lg ${isWithdrawal
                              ? 'text-red-600'
                              : 'text-emerald-600'
                            }`}
                        />
                      ...
                      <div>
                        ...
                        <p
                          className={`mt-1 text-[16px] font-semibold ${isWithdrawal
                              ? 'text-red-600'
                              : 'text-emerald-600'
                            }`}
                        >
